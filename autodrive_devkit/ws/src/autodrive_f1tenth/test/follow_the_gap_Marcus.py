...
class FollowTheGap(Node):
    ...
    def __init__(self):
        ...
        self.steering_filter = MeanFilter(size=20)

        #Fitted parameters of the Follow the gap problem
        self.carDiameter = 0.31		    # cm # https://github.com/f1tenth/f1tenth_gym/blob/4fdb9c7e6fb7c701290f4dc18377d07c1681724f/gym/f110_gym/envs/collision_models.py#L281
        self.tolMaskDisparities = 0.25 	# cm d = carWidth/2 + tolerance # This higher number prevents the car to collide creating a slighter bigger virtual wall, so the car won't collide. 
        self.disparityTreshold = 0.05      # cm #If the n+1 beam is > than n beam for more than 5 meters, it means a "gap" in this region, in other words the car can throttle!
        self.timeToCollision = 27.5 #30.0   	 # s  #Read from the article sent
        self.SafeLateralDistance = 1.0 	# cm
        self.steeringSaturation = 40     # degrees # It is how much the steering control is able to swerve
        self.minSpeedFactor = 10.0     	# 5->20%reduction # values > 1 #Extracted from the article reading
        self.safeCurveExitRatio = 0.1 	# in [0,1] ensure safe exit from curve (do not accelerate until going fully straight)
        self.angleOld = 0
        self.thetaOld = 0
        self.deltaThetaMax = np.radians(90) # Read from article 180 means no limit in steering ratio
        self.speedOld = 0
        self.maxDeltaSpeed = 0.15   	    # %
        self.maxThrotle = 10 #12  		    # % #REad from article
            #Steeirng Control
        self.Kp_steer = 1
        self.Td = 0.2
        self.N = 10
    ...
    def followTheGap(self):
        ...
        #3 For each disparity filter out values out of range:
